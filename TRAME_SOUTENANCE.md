...
**Q5 — Pourquoi le frontend est-il sur une branche séparée ?**
Parce que le backend était le périmètre validé et devait rester stable. Le frontend était une exploration personnelle : je ne voulais pas qu'une modification exploratoire, comme l'ajout de la configuration CORS, déstabilise un backend déjà validé par mon encadrante. C'est l'usage standard des branches de fonctionnalité : isoler ce qui n'est pas stabilisé.
